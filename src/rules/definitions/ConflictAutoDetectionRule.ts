...
export const ConflictAutoDetectionRule: Rule = {
    ...
    evaluate: async (event: GovernanceEvent, context: RuleContext): Promise<RuleResult> => {
        ...
        // Only check if sources are different
        if (previousSource === currentSource) {
            return {
                ruleId: 'TRUST_003', ruleName: 'Conflict Auto-Detection', passed: true, severity: 'INFO',
                reasons: ['Same source update, skipping conflict check'], actionRequired: { type: 'NONE', details: {} }
            };
        }
            ...
            if (current.price && previous.price) {
                ...
                if (pcent > 0.20) {
                    return {
                        ...
                        passed: true, // It passes validation, but we raise a flag
                        severity: 'WARNING',
                        reasons: [`Significant price drift detected (${(pcent * 100).toFixed(1)}%) between sources`],
                        actionRequired: {
                            type: 'FLAG',
                            details: {
                                issue: 'PRICE_DRIFT',
                                msg: `Price changed from ${previous.price} to ${current.price}`
                            }
                        }
                    };
                }
