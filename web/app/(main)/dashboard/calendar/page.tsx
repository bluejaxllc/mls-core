    ...
    return (
        <PageTransition className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">Calendario de Visitas</h2>
                    <p className="text-muted-foreground">Gestiona tus próximas citas y solicitudes.</p>
                </div>
                <div className="flex bg-muted/50 p-1 rounded-lg border">
                    <button
                        onClick={() => setViewRole('agent')}
                        className={`px-3 py-1.5 text-sm rounded-md transition-all ${viewRole === 'agent' ? 'bg-card shadow text-blue-500 font-medium' : 'text-muted-foreground'}`}
                    >
                        Soy Agente (Mis Propiedades)
                    </button>
                    <button
                        onClick={() => setViewRole('visitor')}
                        className={`px-3 py-1.5 text-sm rounded-md transition-all ${viewRole === 'visitor' ? 'bg-card shadow text-blue-500 font-medium' : 'text-muted-foreground'}`}
                    >
                    ...
                    appointments.map((apt, i) => (
                        <AnimatedCard key={apt.id} index={i} className="p-5 border rounded-xl flex flex-col md:flex-row justify-between gap-4 md:items-center group hover:border-blue-200 transition-colors">
                            <div className="flex items-start gap-4">
                                <div className={`p-3 rounded-full flex-shrink-0 ${apt.status === 'CONFIRMED' ? 'bg-green-500/10 text-green-500' :
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${apt.status === 'CONFIRMED' ? 'bg-green-500/10 text-green-500' :
                                            apt.status === 'CANCELLED' ? 'bg-red-500/10 text-red-500' :
                                                'bg-amber-500/10 text-amber-500'
                                            }`}>
                                            {apt.status}
                            ...
                            {viewRole === 'agent' && apt.status === 'PENDING' && (
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="flex items-center gap-1 px-3 py-2 bg-green-500/10 text-green-500 hover:bg-green-500/20 rounded-lg text-sm font-medium transition-colors"
                                    >
                                        <CheckCircle className="h-4 w-4" /> Confirmar
                                    </button>
                                    <button
                                        onClick={() => updateStatus(apt.id, 'CANCELLED')}
                                        className="flex items-center gap-1 px-3 py-2 bg-red-500/10 text-red-500 hover:bg-red-500/20 rounded-lg text-sm font-medium transition-colors"
                                    >
