...
export default function IntelligenceDashboard() {
    ...
    const fetchData = async (page = currentPage) => {
        try {
            ...
            // (Vercel functions CAN'T reach Cloudflare tunnels Ã¢â‚¬â€ must use client-side)
            const proxyUrl = liveData?.proxyUrl || 'https://kate-deposit-diff-develop.trycloudflare.com';
    ...
    const triggerCrawl = async () => {
        ...
        try {
            ...
            if (!statusRes.ok) {
                // Status endpoint failed Ã¢â‚¬â€ skip auth check, ML listings still come via ZenRows in /api/listings/live
                setCrawlStatus('done');
                setCrawlResult('Usando API pÃƒÂºblica');
                return;
            }
            ...
            if (!statusData.oauthConnected) {
                setCrawlResult('API pÃƒÂºblica (conectar OAuth para mÃƒÂ¡s velocidad)');
            }
            ...
            if (crawlData.success) {
                ...
                setCrawlStatus('not_auth');
                setCrawlResult('Token expirado Ã¢â‚¬â€ necesita re-autenticaciÃƒÂ³n');
            } else {
            ...
            setCrawlStatus('error');
            setCrawlResult(e.message || 'Error de conexiÃƒÂ³n');
        }
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        Propiedades detectadas automÃƒÂ¡ticamente por crawlers y agentes de vigilancia.
                    </p>
                </div>
                ...
                <div className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${crawlStatus === 'crawling' ? 'bg-blue-500/10 text-blue-500 border border-blue-500/20' :
                    ...
                    {crawlStatus === 'done' && (
                        <>
                            ...
                            <span>Crawl completado Ã¢â‚¬â€ {crawlResult}</span>
                        </>
                    )}
                    {crawlStatus === 'error' && (
                        <>
                    ...
                    {crawlStatus === 'not_auth' && (
                        <>
                            ...
                            <span>{crawlResult} Ã¢â‚¬â€ <a href="/api/integrations/mercadolibre/auth" className="underline font-semibold">Conectar OAuth</a></span>
                        </>
                    )}
                </div>
            )}
                ...
                <div className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${fbStatus === 'crawling' ? 'bg-purple-500/10 text-purple-500 border border-purple-500/20' :
                    ...
                    {fbStatus === 'done' && (
                        <>
                            ...
                            <span>Facebook Ã¢â‚¬â€ {fbResult}</span>
                        </>
                    )}
                    {fbStatus === 'error' && (
                        <>
            ...
            <section>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <select value={city} onChange={(e) => setCity(e.target.value)} className="w-full h-8 text-xs border border-blue-500/20 rounded-md px-2 bg-muted/50 text-foreground">
                                    ...
                                    <option value="JuÃƒÂ¡rez">JuÃƒÂ¡rez</option>
                                    <option value="Delicias">Delicias</option>
                                    <option value="CuauhtÃƒÂ©moc">CuauhtÃƒÂ©moc</option>
                                </select>
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-muted-foreground">OperaciÃƒÂ³n</label>
                            ...
                            <div className="space-y-1.5">
                                <label className="text-xs font-medium text-muted-foreground">RecÃƒÂ¡maras</label>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-muted/30 rounded-2xl border-2 border-dashed border-blue-500/10">
                        ...
                        <p className="text-sm text-muted-foreground mt-2">Los crawlers se actualizan automÃƒÂ¡ticamente al cargar la pÃƒÂ¡gina.</p>
                    </div>
                ) : (
                    <div className="space-y-8">
                            ...
                            <div className="flex justify-center items-center gap-2 pt-6 pb-2">
                                ...
                                <div className="flex items-center gap-1">
                                    {Array.from({ length: filteredTotalPages }, (_, i) => i + 1)
                                        ...
                                        .map((p, i) =>
                                            p === '...' ? (
                                                <span key={`dots-${i}`} className="px-2 py-1 text-muted-foreground text-sm">Ã¢â‚¬Â¦</span>
                                            ) : (
                                                <button
                                                    key={p}
                        ...
                        <div className="text-center text-xs text-muted-foreground pt-1">
                            PÃƒÂ¡gina {currentPage} de {filteredTotalPages} Ã¢â‚¬Â¢ {filteredListings.length} propiedades
                            {fbStatus === 'done' && ` Ã¢â‚¬Â¢ ${fbResult}`}
                        </div>
                    </div>
                )}
            </section>
