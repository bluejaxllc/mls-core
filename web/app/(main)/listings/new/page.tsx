...
export default function NewListingPage() {
    ...
    const generateAIContent = useCallback(async (
        ...
        try {
            ...
            if (!res || !res.ok) {
                console.error('AI Gen API failed:', res?.status, res?.statusText);
                return;
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!formData.title || !formData.price || !formData.description) {
                alert('Por favor complete los campos requeridos (Título, Precio, Descripción).');
                setLoading(false);
                return;
            }
            ...
            alert('Error al crear la propiedad. Intente nuevamente.');
        } finally {
            setLoading(false);
        }
    };
